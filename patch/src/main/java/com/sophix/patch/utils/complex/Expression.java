package com.sophix.patch.utils.complex;
...
public class Expression
{
    ...
    private boolean isAddSubSymbol(int p)
    {
        ...
        if (ParseNumber.isBaseSymbol(cj))
        { // a pos/neg symbol in scientific notation under certain base
            int pos;
            ...
            if (pos == p + 1)
            { // '+/-' directly followed by non-integer symbol
                return true;
            }
            if (ParseNumber.isBaseSymbol(cj) || (cj >= 'A' && cj <= 'F') || cj == '.')
            { // part of another notation
                return true;
            }
            return false;
        }
    ...
    public Result value(int l, int r, Complex vX)
    {
        ...
        if (s.equals("reg"))
        {
            return new Result(memValue); // reg value
        }
        ...
        { // Constants
            ...
            if (s.equals("e"))
            {
                complexConst = Complex.E; // constant e
            }
            else if (s.equals("π"))
            {
                complexConst = Complex.PI; // constant pi
            }
            else if (s.equals("i"))
            {
                complexConst = Complex.I; // constant i
            }
            else if (s.equals("∞"))
            {
                complexConst = Complex.Inf; // constant Infinity
            }
            else if (s.equals("°"))
            {
                complexConst = new Complex(Math.PI / 180); // degree value
            }
            else if (s.equals("%"))
            {
                complexConst = new Complex(0.01); // percent value
            }
            else
            {
                String constValue = Constants.CONSTANTS_VALUE_MAP.get(s);
                if (constValue != null)
                {
        ...
        try
        {
            ...
            if (s.indexOf('e') >= 0 || s.indexOf('I') >= 0 || s.indexOf('N') >= 0 || s.indexOf('X') >= 0 || s.indexOf('P') >= 0 || s.indexOf('x') >= 0 || s.indexOf('p') >= 0)
            {
                throw new NumberFormatException();
            }

            try
            { // try parse decimal double
                if (s.indexOf('D') >= 0 || s.indexOf('F') >= 0)
                { // forbid double and float sign
                    throw new NumberFormatException();
                }
    ...
    private Result funcValue(int l, int r, Complex vX)
    {
        ...
        switch (funcJump)
        {
            ...
            case Function.PREC + 1:
                ...
                {
                    return new Result(1).setVal(new Complex(1)).setAnswer("设置的精度过高，最大精度是 " + Result.maxPrecision + " 位小数");
                }
    ...
    Result solve(int l, int r, Complex x0, Complex M, int iter)
    {
        ...
        if (r1.norm2() < 1E-20 && v2.norm2() < 1E-20)
        { // 1E-10 precision
        ...
        { // normally no more than 20 iter., but for eq. such as x^.2, more is needed.
            ...
            { // Math Error
                if (M.re == 1.0)
                { // 1.0 is a safely expressed double
    ...
    // general solve
    Result solve(int l, int r, Complex x0)
    { // auto solver
        Result rp;
    ...
    private static final double[] gaussNodes15 = new double[]{ // G15 Nodes
                                                               0.000000000000000000000000000000000e+00, 2.011940939974345223006283033945962e-01, 3.941513470775633698972073709810455e-01, 5.709721726085388475372267372539106e-01, 7.244177313601700474161860546139380e-01, 8.482065834104272162006483207742169e-01, 9.372733924007059043077589477102095e-01, 9.879925180204854284895657185866126e-01
    };

    private static final double[] gaussWeights15 = new double[]{
            2.025782419255612728806201999675193e-01, 1.984314853271115764561183264438393e-01, 1.861610000155622110268005618664228e-01, 1.662692058169939335532008604812088e-01, 1.395706779261543144478047945110283e-01, 1.071592204671719350118695466858693e-01, 7.036604748810812470926741645066734e-02, 3.075324199611726835462839357720442e-02
    };

    Complex gaussIntegrate15(int l, int r, Complex x0, Complex x2)
    { // G15
        ...
        for (int i = 0; i < 7; i++)
        {
            ...
            sum = Complex.add(sum, new Complex((tp[i].re + tn[i].re) * gaussWeights15[i + 1], (tp[i].im + tn[i].im) * gaussWeights15[i + 1]));
        }
    ...
    Result adaptiveIntegrate(int l, int r, Complex x0, Complex x2, Complex lastSum, double TOL, int depth)
    {
        ...
        if (Double.isInfinite(x2.re))
        {
            ...
            double newRe;
            if (aRe <= 1E5)
            { // Avoid precision loss due to large number
    ...
    private Complex permIter(Complex n_, Complex m_)
    { // Gamma(n+1)/Gamma(m+1)
        Complex n, m;
        ...
        { // adapted from iteration
            ...
            if (!ans.isFinite())
            { // invalid value occurred, no need to continue
                break;
            }
            if (!isWorking)
            {
    ...
    private Complex combIter(Complex n_, Complex m_)
    ...
    private Result limitH(int l, int r, Complex x0, Complex h)
    {
        ...
        {
            f0 = new Complex((-f1.re + 24 * f2.re - 81 * f3.re + 64 * f4.re) / 6, (-f1.im + 24 * f2.im - 81 * f3.im + 64 * f4.im) / 6);
        }
    ...
    public Result limit(int l, int r, Complex x0)
    {
        ...
        { // find directions in [0,pi)
            ...
            for (h = 1E-1; h >= 1E-10; h *= 0.9, cnt++)
            {
                ...
                {
                    return new Result(1).setAnswer("未找到极限");
                }
