...
public class Complex
{
    ...
    { // finite complex or Complex Infinity
    ...
    public static Complex div(Complex a, Complex b)
    {
        ...
        if (aNorm > 0 && bNorm == 0)
        {
            return Inf; // pInf==nInf in complex field?
        }
    ...
    private static double[] gammaT = { // constants for Taylor approximation
                                       -0.57721566490153286, 0.9890559953279725, 0.9074790760808862, 0.9817280868344002, 0.9819950689031453, 0.9931491146212761
    };

    public static Complex gamma(Complex c)
    { // Lanczos approximation + Taylor series
        ...
        if (c.re < -310)
        { // guarantee result in double field
            if (c.re == Double.NEGATIVE_INFINITY)
            {
                if (c.im == 0)
                {
        ...
        else if (Math.abs(c.re) < 1E-3 && Math.abs(c.im) < 1E-2)
        { // Taylor series, deal with value REALLY near the pole 0
            result = new Complex(0);
        ...
        else if (c.re < 0.5 && Math.abs(c.im) <= 220)
        { // Reflection formula(more precise), deal with value near the pole 0
